...
pub async fn create_pr(
    ...
    let branch = if branch.is_empty() {
        ...
        if b == "HEAD" {
            let branches = gh(&["branch", "--format=%(refname:short)", "--points-at=HEAD"])
                .unwrap_or_default();
    ...
    if branch.is_empty() || branch == "HEAD" {
        anyhow::bail!(
            "Cannot determine current branch name. Run from a named branch or set BRANCH env var."
        );
    }

    let _ = gh(&["push", "--set-upstream", "origin", &branch]);
    let mut args = vec![
        "pr", "create", "--repo", &repo, "--title", title, "--head", &branch,
    ];
    if !body.is_empty() {
        args.push("--body");
        args.push(body);
...
pub async fn push_commits(pr: u64, dispatch_agent: Option<&str>) -> Result<()> {
    let branch = gh(&["rev-parse", "--abbrev-ref", "HEAD"]).unwrap_or_else(|_| "HEAD".to_string());
...
pub async fn create_sub_issue(
    ...
    let out = gh(&[
        "issue", "create", "--repo", &repo, "--title", title, "--body", &full_body,
    ])?;
    let num = out.rsplit('/').next().unwrap_or("?");
    eprintln!(
        "atoma-github create-sub-issue: created issue #{} — {}",
        num, out
    );
    Ok(())
}
...
pub async fn fetch_events(
    ...
    let out = gh(&[
        "api",
        &format!("repos/{}/{}/{}/timeline", repo, resource, number),
        "--jq",
        ".",
    ])?;
    ...
    eprintln!(
        "atoma-github fetch-events: fetched timeline for {} #{}",
        type_, number
    );
    Ok(())
}
...
pub async fn build_context(
    ...
    let events = std::fs::read_to_string(events_path).context("Failed to read events file")?;
    ...
    eprintln!(
        "atoma-github build-context: built context for {}",
        agent_name
    );
    Ok(())
}
